...
class EyeTribeServer(object):
    ...
    def __init__(self, host="localhost", port=6555, buffer_size=4096):
        ...
        buffer_size -- the number of bytes the socket will recv at once.
        ...
        self.listener_thr = ListenerThread(
                recv_function=lambda: self.socket.recv(buffer_size),
                q=self.raw_q
                )
    ...
    def version(self, int_):
        self._set_value(u'version', int_)
    ...
    def screenindex(self, int_):
        self._set_value(u'screenindex', int_)
    ...
    def screenresw(self, int_):
        self._set_value(u'screenresw', int_)
    ...
    def screenresh(self, int_):
        self._set_value(u'screenresh', int_)
    ...
    def screenpsyw(self, float_):
        self._set_value(u'screenpsyw', float_)
    ...
    def screenpsyh(self, float_):
        self._set_value(u'screenpsyh', float_)
    ...
    def screenres(self, arg1, arg2=None):
        ...
        if type(arg1) == list or type(arg1) == tuple:
            if len(arg1) == 2:
                self._set_values(screenresw=arg1[0], screenresh=arg1[1])
            else:
                # error
                pass
        elif arg2 is not None:
            self._set_values(screenresw=arg1, screenresh=arg2)
    ...
    def get_avg_xy(self):
        ...
        """
        frame_ = self.frame
        return frame_[u'avg'][u'x'], frame_[u'avg'][u'y']  # returns a tuple
    ...
    def get_pupil_locations(self):
        ...
        return lefteye, riteeye  # returns a tuple
    ...
    def est_cpu_minus_tracker_time(
        ...
        for i in range(num_samples):
            ...
            if tracker_times[i] == tracker_times[i+1] or cpu_td.total_seconds() > cpu_diff_tolerance:
...
class EyeTribeError(Exception):
    ...
    err_msg_dict = {
            1001: 'Cannot estimate cpu-tracker time difference while in push mode.',
            1002: 'Cannot estimate cpu-tracker time difference unless tracker is properly connected',
            2001: 'You cannot set this value, it is determined by the EyeTribeServer.',
            3001: 'You cannot use the get method in an EyeTribeQueue. Use the get_item method instead.'
            }
